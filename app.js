...
function resposta (req, res) {
	var notfound = __dirname + '/notfound.html';
	var arquivo = "";
	if(req.url == "/"){
	...
	fs.readFile(arquivo,
		function (err, data) {
			if (err) {
				res.writeHead(404);
				return res.end(getnotfound(notfound));
			}
...
function getnotfound(notfound){
	fs.readFile(notfound, function(error, resp){
		if(error){
			return "There is something wrong, a file is missing.";
		}
		return resp;
	});
}
...
io.on("connection", function(socket){
	...
	socket.on("entrar", function(apelido, callback){
		if(!(apelido in usuarios)){
			...
			console.log("LOG> A User has been connected. Name: '" + apelido + "'." );
			callback(true);
		}else{
			callback(false);
		}
	...
	socket.on("enviar mensagem", function(dados, callback){
		...
		console.log("LOG> '"+ socket.apelido +"' " + "has sent a message." );
		callback();
	});
	...
	socket.on("disconnect", function(){
		...
		var obj_mensagem = {msg: mensagem, tipo: 'sistema'};
		console.log("LOG> '"+ socket.apelido +"' " + " has left the server." );
